...
struct node {
   ...
   if the node is a directory else it is null*/
   DynArray_T oDChildren;
   /*holds the contents of a file if the node is a file
      else it is NULL*/
   void* pvFileContents;
   
   /*a boolean to check whether a Node is a file*/
   boolean isFile;
...
Node_T *poNResult, int isFile, void* pvFileContents ) {
   struct node *psNew;
   ...
   /*make sure if the node is a dir that it has no contents*/
   if(!isFile) assert(pvFileContents == NULL);
   ...
   if(!isFile) {
      ...
      psNew->pvFileContents = pvFileContents;
      psNew->isFile = TRUE;
